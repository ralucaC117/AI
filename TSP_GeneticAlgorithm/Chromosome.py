...
class Chromosome:
    def __init__(self, problParam=None):
        ...
        self.__repres = generateARandomPermutation(self.__problParam['noOfNodes'])
        self.__fitness = 0.0
    ...
    def crossover(self, c):
        # order XO
        pos1 = randint(1, self.__problParam['noOfNodes'] - 1)
        pos2 = randint(1, self.__problParam['noOfNodes'] - 1)
        if pos2 < pos1:
            pos1, pos2 = pos2, pos1
        k = 1
        ...
        for el in c.__repres[pos2:] + c.__repres[:pos2]:
            if el not in newrepres:
                if len(newrepres) < self.__problParam['noOfNodes'] - pos1:
                    newrepres.append(el)
                else:
